...
void MainScreenCommunication::ReceiveCallback(const CanId &canId, const uint8_t *data, uint8_t length)
{
    ...
    if (canId.type == CanMsgType::CAN_AIRRIDE_LOG)
    {
...
String MainScreenCommunication::CreateLogMessage(bool front, float startPressure, float endPressure, float startTankPressure,
